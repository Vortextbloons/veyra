...
pub struct CharacterGroupDbState {
    app: tauri::AppHandle,
    db: crate::shared::db_utils::DbSlot<CharacterGroupDb>,
}
...
impl CharacterGroupDbState {
        ...
        F: FnOnce(&Connection) -> Result<T, String>,
    {
        let db = match self
            .db
            .get_or_init(|| CharacterGroupDb::init(&self.app).map(Arc::new))
        {
            Ok(db) => Arc::clone(db),
...
impl crate::shared::db_utils::DbConnectionState for CharacterGroupDbState {
