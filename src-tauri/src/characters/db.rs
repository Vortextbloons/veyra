...
pub struct CharacterDbState {
    app: tauri::AppHandle,
    db: crate::shared::db_utils::DbSlot<CharacterDb>,
}
...
impl CharacterDbState {
        ...
        F: FnOnce(&Connection) -> Result<T, String>,
    {
        let db = match self
            .db
            .get_or_init(|| CharacterDb::init(&self.app).map(Arc::new))
        {
            Ok(db) => Arc::clone(db),
...
impl crate::shared::db_utils::DbConnectionState for CharacterDbState {
