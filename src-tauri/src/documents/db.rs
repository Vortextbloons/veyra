...
pub struct DocumentDbState {
    app: tauri::AppHandle,
    db: crate::shared::db_utils::DbSlot<DocumentDb>,
}
...
impl DocumentDbState {
        ...
        F: FnOnce(&Connection) -> Result<T, String>,
    {
        let db = match self
            .db
            .get_or_init(|| DocumentDb::init(&self.app).map(Arc::new))
        {
            Ok(db) => Arc::clone(db),
...
impl crate::shared::db_utils::DbConnectionState for DocumentDbState {
