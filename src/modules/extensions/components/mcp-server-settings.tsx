...
export function McpServerSettings() {
  ...
  const setServerEnabled = (server: McpServerRecord, enabled: boolean) => {
    const next = {
      ...server,
      enabled,
      health: server.health === "disabled" ? "degraded" : server.health,
      updatedAt: new Date().toISOString(),
    };
    save(next);
    if (next.health !== "ready") void connect(next);
  };
  ...
  return <div className="rounded-xl border border-[var(--color-border)] bg-[var(--color-panel)]">
    ...
    <div className="divide-y divide-[var(--color-border)]">{servers.length === 0 ? <p className="px-4 py-8 text-center text-[11px] text-[var(--color-text-dim)]">No MCP servers configured.</p> : servers.map((server) => <div key={server.id} className="flex items-center gap-3 px-4 py-3"><Activity className={`size-4 ${server.enabled && server.health === "ready" ? "text-emerald-400" : server.health === "failed" ? "text-rose-300" : "text-[var(--color-text-dim)]"}`} /><div className="min-w-0 flex-1"><p className="text-[12px] text-white">{server.name}</p><p className="truncate text-[10px] text-[var(--color-text-dim)]">{server.description} · {server.capabilityCount.tools} tools, {server.capabilityCount.resources} resources, {server.capabilityCount.prompts} prompts</p></div><button type="button" onClick={() => setServerEnabled(server, !server.enabled)} className={`shrink-0 rounded-md px-2 py-1 text-[10px] ${server.enabled ? "bg-emerald-500/10 text-emerald-300" : "bg-white/5 text-[var(--color-text-dim)]"}`}>{server.enabled ? "Enabled" : "Disabled"}</button><button type="button" onClick={() => { setInspecting(server.id); setTab("tools"); }} className="text-[10px] text-violet-200 hover:text-white">Inspect</button><button type="button" disabled={!server.enabled} onClick={() => void connect(server)} className="p-1.5 text-[var(--color-text-dim)] hover:text-white disabled:cursor-not-allowed disabled:opacity-35" title="Refresh capabilities"><RefreshCw className="size-3.5" /></button><button type="button" onClick={() => remove(server.id)} className="p-1.5 text-[var(--color-text-dim)] hover:text-rose-300" title="Remove server"><Trash2 className="size-3.5" /></button></div>)}</div>
    {selected && <div className="border-t border-[var(--color-border)] p-4"><div className="mb-3 flex items-center justify-between"><div><p className="text-[12px] font-medium text-white">{selected.name}</p><p className="text-[10px] text-[var(--color-text-dim)]">{selected.transport} · {selected.health}</p></div><button type="button" onClick={() => setInspecting(null)} className="text-[10px] text-[var(--color-text-dim)]">Close</button></div><div className="mb-3 flex gap-1 border-b border-[var(--color-border)] pb-2">{(["tools", "resources", "prompts", "logs", "permissions", "configuration"] as InspectorTab[]).map((item) => <button key={item} type="button" onClick={() => setTab(item)} className={`rounded px-2 py-1 text-[10px] capitalize ${tab === item ? "bg-violet-400/15 text-violet-100" : "text-[var(--color-text-dim)] hover:text-white"}`}>{item}</button>)}</div>{message && <p className="mb-2 text-[10px] text-emerald-300">{message}</p>}
      ...
      {tab === "configuration" && <div className="space-y-2"><label className="flex items-center justify-between rounded-md border border-white/[0.06] px-2 py-2 text-[11px] text-white">Enable this server by default<input type="checkbox" checked={selected.enabled} onChange={(event) => setServerEnabled(selected, event.target.checked)} className="accent-violet-500" /></label><p className="text-[10px] text-[var(--color-text-dim)]">Disabled servers stay available in the chat MCP menu, but start off in new chats.</p><pre className="max-h-48 overflow-auto rounded-md bg-black/20 p-2 text-[10px] text-[var(--color-text-dim)]">{JSON.stringify({ transport: selected.transport, endpoint: selected.endpoint, executable: selected.executable, arguments: selected.arguments, timeoutMs: selected.timeoutMs, projectIds: selected.projectIds }, null, 2)}</pre></div>}
    </div>}
  </div>;
}
